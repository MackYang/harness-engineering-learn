...
**关键洞察**: 现在 Loop 不再是 "写一堆 bash" 了。一年前你需要写一堆 bash 脚本然后永远维护它。现在五大构件已经内置于 Codex 和 Claude Code 产品中。两者的形状相同，所以你可以设计一个不依赖具体工具的 loop。

## Loop 的五大构件 + 一个记忆

| 构件 | Loop 中的角色 | Codex | Claude Code |
|------|-------------|-------|-------------|
| Automations | 定时发现 + 分诊 | Automations tab: 选项目、prompt、频率、环境; /goal | /loop, /goal, cron, hooks, GitHub Actions |
| Worktrees | 隔离并行特性 | 内置 worktree per thread | git worktree, --worktree, isolation: worktree |
| Skills | 编码项目知识 | Agent Skills (SKILL.md), $name 调用 | Agent Skills (SKILL.md) |
| Plugins/Connectors | 连接真实工具 | Connectors (MCP) + plugins | MCP servers + plugins |
| Sub-agents | 构思和验证 | .codex/agents/ (TOML) | .claude/agents/, agent teams |
| State | 追踪进度 | Markdown 或 Linear via connector | Markdown (AGENTS.md, progress files) 或 Linear via MCP |
...
- `/loop` 按间隔重跑
- `/goal` 持续运行直到条件满足，由**独立小模型**判断完成（maker/checker 分离应用到停止条件本身）
- Cron tasks, hooks, GitHub Actions

**Session 内原语**: `/goal` 在每轮后由一个独立小模型检查是否完成，写代码的 agent 不批改自己的作业。
...
**注意**: Worktrees 解决机械碰撞，但你的审查带宽才是并行上限，不是工具。这被称为"编排税"（Orchestration Tax）。
...
**Skill vs Plugin 的区别**: Skill 是**创作格式**（authoring format），Plugin 是**分发格式**（distribution format）。跨仓库分享或打包多个 skills 时，包装为 plugin。
...
> The connectors are the reason the loop can act inside your actual environment instead of just telling you what it would do if it could.
...
**Token 成本注意**: 每个 sub-agent 有独立的模型和工具调用开销，所以在值得为第二意见付费的地方才使用。
...
*完整学习对比: 2026-06-26 — 与原文逐段比对后补充了构件对照表、Skill vs Plugin 区别、编排税、Token 成本提醒等遗漏要点*
